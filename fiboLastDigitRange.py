...
if __name__ == '__main__':
    ...
    if (m-1) < period :
        if m !=0:
            totalSum_m = get_sum(10, m-1)
        else :
            totalSum_m=get_sum(10,m)
    else :
        q_m = (m-1)/period
        r_m = (m-1)%period
        sum_q_periods = (q_m * sum_per_period)%10
        remaining_sum_m = get_sum(10,r_m)
        totalSum_m = sum_q_periods + remaining_sum_m
        #print("remaining sum:",remaining_sum)

    if n < period :
        totalSum_n = get_sum(10,n)
    else :
        q_n = n/period
        r_n = n%period
        sum_q_periods = (q_n * sum_per_period)%10
        remaining_sum_n = get_sum(10,r_n)
        totalSum_n = sum_q_periods + remaining_sum_n
        #print("remaining sum:",remaining_sum)
        
    if totalSum_n < totalSum_m:
        totalSum = (10+totalSum_n)-totalSum_m
    else :
        totalSum = totalSum_n-totalSum_m
